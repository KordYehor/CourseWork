...
void DepartmentList::PrintInfo() {
	...
	std::cout << stringLeftAlign(ML.getMaxNameL(), "Название отдела") << "|" << stringLeftAlign(20, "Кол-во сотрудников") << "|" << stringLeftAlign(32, "Сумарная з/п") << "|" << stringLeftAlign(20, "Средняя з/п") << "\n";
	std::cout << stringLeftAlign(70 + ML.getMaxNameL(), "-", '-') << "\n";
	while (p) {
		if (p->list.getSize() != 0) {
			std::cout << stringLeftAlign(ML.getMaxNameL(), p->getDepartmentName()) << "|" << stringLeftAlign(20, std::to_string(p->list.getSize())) << "|" << stringLeftAlign(32, std::to_string((int)p->list.getSalarySumm()) + " Грн.") << "|" << stringLeftAlign(20, std::to_string((int)p->list.getSalarySumm()/p->list.getSize()) + " Грн.") << "\n";
			std::cout << stringLeftAlign(70 + ML.getMaxNameL(), "-", '-') << "\n";
		}
		else {
			std::cout << stringLeftAlign(ML.getMaxNameL(), p->getDepartmentName()) << "|" << stringLeftAlign(20, std::to_string(p->list.getSize())) << "|" << stringLeftAlign(32, std::to_string((int)p->list.getSalarySumm()) + " Грн.") << "|" << stringLeftAlign(20, std::to_string((int)p->list.getSalarySumm()) + " Грн.") << "\n";
			std::cout << stringLeftAlign(70 + ML.getMaxNameL(), "-", '-') << "\n";
		}
