...
Choice List::ChooseFind() {
	...
	if (choice == -1) {
		system("cls");
		std::cout << " Ошибка!\n Вы ввели недопустимый символ при выборе действия!\n Для выбора операции введите числа 1-12 или 13, для выхода\n";
...
void List::PrintOne(Node* p,bool mode) {
	if (mode == false) {
		std::cout << stringLeftAlign(ML.getMaxIdL(), (std::string)std::to_string(p->data.getID())) << "|" << stringLeftAlign(ML.getMaxNameL(), p->data.getName()) << "|" << stringLeftAlign(ML.getMaxPosL(), p->data.getPosition()) << "|" << stringLeftAlign(12, std::to_string(p->data.getWeekdays()) + "/" + std::to_string(p->data.getDayOffs())) << "|" << stringLeftAlign(ML.getMaxNumL(), p->data.getNumber()) << "|" << stringLeftAlign(ML.getMaxSalaryL() + 5, std::to_string(p->data.getSalary()) + " грн. ") << "|" << stringLeftAlign(8, std::to_string(p->data.getVacation())) << "|"
			<< stringLeftAlign(18, "...") << "\n";
		std::cout << stringLeftAlign(18 + 8 + 5 + 12 + ML.getMaxIdL() + ML.getMaxNameL() + ML.getMaxNumL() + ML.getMaxPosL() + ML.getMaxSalaryL(), "-", '-') << "\n";
	}
	else if (mode == true) {
		std::cout << stringLeftAlign(ML.getMaxIdL(), std::to_string(p->data.getID())) << "|" << stringLeftAlign(ML.getMaxNameL(), p->data.getName()) << "|" << stringLeftAlign(ML.getMaxPosL(), p->data.getPosition()) << "|" << stringLeftAlign(12, std::to_string(p->data.getWeekdays()) + "/" + std::to_string(p->data.getDayOffs())) << "|" << stringLeftAlign(ML.getMaxNumL(), p->data.getNumber()) << "|" << stringLeftAlign(ML.getMaxSalaryL() + 5, std::to_string(p->data.getSalary()) + " грн. ") << "|" << stringLeftAlign(8, std::to_string(p->data.getVacation())) << "|";
		p->data.PrintExp(32 + ML.getMaxIdL() + ML.getMaxNameL() + ML.getMaxNumL() + ML.getMaxPosL() + ML.getMaxSalaryL());
		std::cout << stringLeftAlign(18 + 8 + 5 + 12 + ML.getMaxIdL() + ML.getMaxNameL() + ML.getMaxNumL() + ML.getMaxPosL() + ML.getMaxSalaryL(), "-", '-') << "\n";
	}
}
...
void List::Print(int tumblr, int param1,int param2, std::string strParam1, bool mode) { // mode == false - отобарзить записи без опыта; true - с опытом
	...
	Node* p = head;
	std::cout << stringLeftAlign(ML.getMaxIdL(), "ID") << "|" << stringLeftAlign(ML.getMaxNameL(), "Имя") << "|" << stringLeftAlign(ML.getMaxPosL(), "Должность") << "|" << stringLeftAlign(12, "График Р/В") << "|" << stringLeftAlign(ML.getMaxNumL(), "Номер телефона") << "|" << stringLeftAlign(ML.getMaxSalaryL() + 5, "Зарплата") << "|" << stringLeftAlign(8, "Отпуск") << "|" << stringLeftAlign(18, "Опыт") << "\n" << stringLeftAlign(18+8+5+12+ML.getMaxIdL() +ML.getMaxNameL() +ML.getMaxNumL() +ML.getMaxPosL() +ML.getMaxSalaryL(), "-", '-') << "\n";
	switch (tumblr) {
	case PRINT_ALL:
		while (p) {
			PrintOne(p, mode);
